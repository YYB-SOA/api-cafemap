...
describe 'Appraise Info Service (CafeNomad) Integration Test' do
    ...
    describe 'Mining a info (CafeNomad)' do
      before do
      ...
      it 'HAPPY: should mine for a api call of an given cafenomad info data' do
        ...
        aa =  CafeMap::Database::InfoOrm.where(city: input[:city])
        
        
        # gh_project = CafeMap::Service::MiningInfo.new(GITHUB_TOKEN).find(USERNAME, PROJECT_NAME)
        # CafeMap::Repository::For.entity(gh_project).create(gh_project)
